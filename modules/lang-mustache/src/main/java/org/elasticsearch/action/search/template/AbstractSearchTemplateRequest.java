...
public abstract class AbstractSearchTemplateRequest<Request extends AbstractSearchTemplateRequest<Request>> extends ActionRequest<Request> {

    private static ObjectParser<AbstractSearchTemplateRequest<?>, ParseFieldMatcherSupplier> PARSER;
    static {
        PARSER = new ObjectParser<>("search_template");
        PARSER.declareField((parser, request, s) -> request.setScriptParams(parser.map()), new ParseField("params"), ValueType.OBJECT);
        ...
        }, new ParseField("inline", "template"), ValueType.OBJECT_OR_STRING);
    }
    ...
    public static <R extends AbstractSearchTemplateRequest<R>> R fromXContent(BytesReference bytes, R request) throws IOException {
        try (XContentParser parser = XContentHelper.createParser(bytes)) {
            return (R) PARSER.parse(parser, request, () -> ParseFieldMatcher.STRICT);
        }
    }
}
